...
# The plaque code might be around `CONTACT ME`
content = content.replace(
    'EMMANUEL BARAKA<br/>\n                   <span className="text-[#1a110c] text-[6px] md:text-[7px]">CONTACT ME</span>',
    'BOOK<br/>\n                   <span className="text-[#1a110c] text-[8px] md:text-[9px]">APPOINTMENT</span>'
)
# Just in case the layout was already modified by `fix_phase7.py`:
content = content.replace(
    'EMMANUEL BARAKA<br/>\n                   <span className="text-[#1a110c] text-[6px] md:text-[7px]">CONTACT ME</span>',
    'BOOK<br/>\n                   <span className="text-[#1a110c] text-[8px] md:text-[9px]">APPOINTMENT</span>'
)
# Wait, let's use a regex to replace EMMANUEL BARAKA.*CONTACT ME with BOOK APPOINTMENT
content = re.sub(
    r'EMMANUEL BARAKA<br/>\s*<span className="text-\[#1a110c\] text-\[6px\] md:text-\[7px\]">CONTACT ME</span>',
    r'BOOK<br/>\n                   <span className="text-[#1a110c] text-[8px] md:text-[10px] leading-tight">APPOINTMENT</span>',
    content
)

# 2. Move Light Switch down (from top-36 md:top-48 to top-64)
...
# We will replace this block with an enhanced 3D rug/desk
enhanced_rug = """            {/* The Main Long Desk (3D Extruded) */}
            <div className="absolute bottom-12 md:bottom-16 w-[85%] max-w-4xl h-32 md:h-40 rounded-t-sm bg-gradient-to-b from-[#4a1c18] to-[#2b0f0c] shadow-[0_30px_50px_rgba(0,0,0,0.9)] flex flex-col items-center opacity-100 z-10 [transform-style:preserve-3d]">
               ...
               <div className="w-[96%] h-[80%] mt-2 border border-[#94392e]/40 rounded-sm flex items-center justify-center bg-[#3a1510]/30 relative">
                  ...
                  <div className="absolute bottom-4 right-8 md:right-16 w-12 md:w-16 h-8 md:h-10 bg-[#111] rounded shadow-lg border-t-2 border-[#333] flex flex-col items-center justify-center rotate-[15deg]">
...
"""
# First, let's find the rug block in the content.
rug_match = re.search(r'            \{/\* Center Persian Rug \*/\}.*?            \{/\* Inlaid Brass Footer Plaque \*/\}', content, re.DOTALL)
if rug_match:
    content = content[:rug_match.start()] + enhanced_rug + "            {/* Inlaid Brass Footer Plaque */}" + content[rug_match.end():]

# 4. Animate hanging bulb out of view on scroll to Room 2
# In LAYER 4 (HUD), we have the bulb. We need to attach style={{ y: bulbScrollY, opacity: bulbOpacity }}
# But wait, we need to create `bulbScrollY` and `bulbOpacity` in the component using `useTransform`.
# Let's insert the useTransform hooks.
...
# Find the bulb div in HUD
bulb_start = '{/* Top-Right Hanging Bulb Indicator */}'
...
# Replace the closing div of the bulb
