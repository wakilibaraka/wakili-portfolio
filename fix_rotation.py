import re

with open("src/components/OfficeRoom.tsx", "r") as f:
    content = f.read()

# Change scroll logic from rotateX to rotateY
# Find the useScroll block
old_scroll_logic = """  // Multi-Room Scroll Logic
  const { scrollYProgress } = useScroll();
  const scrollRoom1RotateX = useTransform(scrollYProgress, [0, 1], [0, 90]);
  const scrollRoom1Z = useTransform(scrollYProgress, [0, 1], [0, -500]);
  const scrollRoom1Opacity = useTransform(scrollYProgress, [0, 0.4], [1, 0]);
  
  const scrollRoom2RotateX = useTransform(scrollYProgress, [0, 1], [-90, 0]);
  const scrollRoom2Z = useTransform(scrollYProgress, [0, 1], [-500, 0]);
  const scrollRoom2Opacity = useTransform(scrollYProgress, [0.6, 1], [0, 1]);"""

new_scroll_logic = """  // Multi-Room Scroll Logic (Revolving Door on Y Axis)
  const { scrollYProgress } = useScroll();
  
  // Room 1 (Reception): Rotates to the right (-90deg on Y axis)
  const scrollRoom1RotateY = useTransform(scrollYProgress, [0, 1], [0, -90]);
  const scrollRoom1Z = useTransform(scrollYProgress, [0, 1], [0, -200]);
  const scrollRoom1Opacity = useTransform(scrollYProgress, [0, 0.6], [1, 0]);
  
  // Room 2 (Office): Rotates in from the left (90deg to 0 on Y axis)
  const scrollRoom2RotateY = useTransform(scrollYProgress, [0, 1], [90, 0]);
  const scrollRoom2Z = useTransform(scrollYProgress, [0, 1], [-200, 0]);
  const scrollRoom2Opacity = useTransform(scrollYProgress, [0.4, 1], [0, 1]);"""

content = content.replace(old_scroll_logic, new_scroll_logic)

# Replace the style bindings for Room 1 wrapper
old_r1_style = "style={{ rotateX: scrollRoom1RotateX, z: scrollRoom1Z, opacity: scrollRoom1Opacity }}"
new_r1_style = "style={{ rotateY: scrollRoom1RotateY, z: scrollRoom1Z, opacity: scrollRoom1Opacity }}"
content = content.replace(old_r1_style, new_r1_style)

# Replace the style bindings for Room 2 wrapper
old_r2_style = "style={{ rotateX: scrollRoom2RotateX, z: scrollRoom2Z, opacity: scrollRoom2Opacity }}"
new_r2_style = "style={{ rotateY: scrollRoom2RotateY, z: scrollRoom2Z, opacity: scrollRoom2Opacity }}"
content = content.replace(old_r2_style, new_r2_style)

with open("src/components/OfficeRoom.tsx", "w") as f:
    f.write(content)
